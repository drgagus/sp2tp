...
function Diagnosa(props) {
    ...
    const getDiagnosa = async () =>{
        try{
            let response = await axios.get(`/puskesmas/diagnosa/${diagnosa}`)
...
if (document.getElementById('listOfDiagnosa')) {
    var item = document.getElementById('listOfDiagnosa')
    ReactDOM.render(<Diagnosa endpoint={item.getAttribute('endpoint')}/>, document.getElementById('listOfDiagnosa'));
}
